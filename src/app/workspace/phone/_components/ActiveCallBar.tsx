...
export function ActiveCallBar() {
  ...
  return (
    <div className="fixed bottom-[calc(5.5rem+env(safe-area-inset-bottom,0px))] left-0 right-0 z-[43] px-4 pb-2 sm:px-5">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-3 rounded-2xl border border-sky-400/40 bg-gradient-to-r from-blue-950 via-slate-900 to-blue-950 px-3 py-2.5 text-sky-50 shadow-xl shadow-blue-950/40">
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold">{activeRemoteLabel ?? "On call"}</p>
          <p className="text-[11px] font-medium text-sky-200">Duration {formatDuration(durationSec)}</p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <button
            ...
            className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-sky-400/35 text-sky-200 opacity-70"
          >

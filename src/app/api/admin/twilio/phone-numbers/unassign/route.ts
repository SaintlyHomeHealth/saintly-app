...
export async function POST(req: Request) {
  ...
  if (row.status === "retired") {
    return NextResponse.json({ error: "Number is retired." }, { status: 400 });
  }
  ...
  await logTwilioNumberAssignment(supabaseAdmin, {
    ...
    reason: typeof body.reason === "string" ? body.reason.trim().slice(0, 500) : "unassign",
  });
  ...
  if (upErr) {
    console.warn("[api/admin/twilio/phone-numbers/unassign]:", upErr.message);

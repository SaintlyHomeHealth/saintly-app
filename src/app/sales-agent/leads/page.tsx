...
export default async function SalesAgentLeadsDashboardPage() {
  ...
  if (error) {
    console.warn("[sales-agent/leads] load:", error.message);
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-slate-900">Your leads</h2>
                ...
                {leads.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50/80">
                    <td className="px-4 py-2.5">
                      <Link href={`/sales-agent/leads/${row.id}`} className="font-medium text-sky-700 hover:underline">

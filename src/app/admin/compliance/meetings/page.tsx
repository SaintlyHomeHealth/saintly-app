...
import { labelFrom, MEETING_TYPES } from "@/lib/compliance/constants";
import { complianceContext } from "@/lib/compliance/page-context";
import { complianceHref, periodLabel } from "@/lib/compliance/period";
import { listMeetings, listNaMarks, readCompliance } from "@/lib/compliance/queries";
...
export default async function MeetingsPage({
  ...
  const loaded = await readCompliance(() =>
    Promise.all([listMeetings(ctx.year, ctx.quarter), listNaMarks(ctx.year, ctx.quarter)])
  );
  if (!loaded.ok) {
    return (
      <AdminPageShell>
  ...
  const [meetings, marks] = loaded.data;
  const na = marks.find((mark) => mark.item_key === "meetings");
      ...
      <AdminPageHeader
        ...
        actions={
          <div className="flex flex-wrap items-end justify-between gap-3">
            <PeriodPicker year={ctx.year} quarter={ctx.quarter} years={ctx.years} path="/admin/compliance/meetings" />
            <Link
              href={complianceHref("/admin/compliance/meetings/new", ctx.year, ctx.quarter)}
      ...
      <div className="space-y-3">
        {meetings.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-slate-300 bg-white px-5 py-8 text-slate-600">
            No meetings saved for this quarter.
          </p>
        ) : (
          meetings.map((meeting) => (
            <Link
              key={meeting.id}
              href={`/admin/compliance/meetings/${meeting.id}`}
              className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-5 py-4"
            >
              <span>
                <span className="block text-lg font-semibold text-slate-900">
                  {labelFrom(MEETING_TYPES, meeting.meeting_type)}
                </span>
                <span className="text-sm text-slate-500">
                  {meeting.meeting_date
                    ? formatAppDate(`${meeting.meeting_date}T12:00:00-07:00`)
                    : "No date yet"}
                </span>
              </span>
              <span className="text-sm font-bold uppercase text-slate-600">{meeting.status}</span>
            </Link>
          ))
        )}
      </div>

...
type ContractReviewFormState = {
  review_date: string;
  compensation_reviewed: boolean;
  duties_reviewed: boolean;
...
type FormHistoryRow = {
  ...
  form_data?: ContractReviewFormState | null;
};
...
const DEFAULT_FORM: ContractReviewFormState = {
  review_date: new Date().toISOString().slice(0, 10),
  compensation_reviewed: false,
  duties_reviewed: false,
...
export default function ContractAnnualReviewForm({
  employeeId,
  complianceEventId,
  ...
  const [form, setForm] = useState<ContractReviewFormState>(DEFAULT_FORM);
  ...
  const completed = useMemo(
    () =>
      [form.review_date, form.reviewed_by].every(
        (value) => typeof value === "string" && value.trim().length > 0
      ) &&
      form.compensation_reviewed &&
      form.duties_reviewed &&
  ...
  async function save(nextStatus: "draft" | "finalized") {
    if (nextStatus === "finalized" && !completed) {
      setError("Complete required review items before finalizing.");
      return;
    }
    ...
    const payload = {
      ...
      form_type: "contract_annual_review",
      form_title: "Contract Annual Review",
      status: nextStatus,
    ...
    <div className="space-y-6 p-6">
      <div className="overflow-hidden rounded-[28px] border border-sky-100 bg-gradient-to-br from-sky-50 via-white to-cyan-50 shadow-[0_20px_60px_-25px_rgba(2,132,199,0.35)]">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">
                Complete yearly contract review items, save progress, then finalize.
              </p>
            </div>
        ...
        <div className="space-y-6 px-6 py-6 md:px-8">
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-700">Review Date</label>
            <input
              type="date"
          ...
          <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
            <label className="flex items-center gap-3 text-sm font-semibold text-slate-700">
              <input
                type="checkbox"
                checked={form.compensation_reviewed}
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, compensation_reviewed: e.target.checked }))
                }
                disabled={status === "finalized"}
                className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
              />
              Compensation terms reviewed
            </label>
            <label className="flex items-center gap-3 text-sm font-semibold text-slate-700">
              <input
                type="checkbox"
                checked={form.duties_reviewed}
                onChange={(e) => setForm((prev) => ({ ...prev, duties_reviewed: e.target.checked }))}
                disabled={status === "finalized"}
                className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
              />
              Duties and scope reviewed
            </label>
            <label className="flex items-center gap-3 text-sm font-semibold text-slate-700">
              <input
                type="checkbox"
                checked={form.policy_acknowledged}
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, policy_acknowledged: e.target.checked }))
                }
                disabled={status === "finalized"}
                className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
              />
              Policy acknowledgement confirmed
            </label>
          </div>
          ...
          {event ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
              <p>
                ...
                {event.event_title || "Contract Annual Review"}
              </p>
              <p className="mt-1">
        ...
        <div className="mt-4 space-y-3">
          {history.length === 0 ? (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">
              No contract annual review history yet.
            </div>
          ) : (

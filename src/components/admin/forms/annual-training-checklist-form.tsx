...
type TrainingChecklistFormState = {
  checklist_date: string;
  modules_reviewed: boolean;
  policy_updates_reviewed: boolean;
...
type FormHistoryRow = {
  ...
  form_data?: TrainingChecklistFormState | null;
};
...
const DEFAULT_FORM: TrainingChecklistFormState = {
  checklist_date: new Date().toISOString().slice(0, 10),
  modules_reviewed: false,
  policy_updates_reviewed: false,
...
export default function AnnualTrainingChecklistForm({
  employeeId,
  complianceEventId,
  ...
  const [form, setForm] = useState<TrainingChecklistFormState>(DEFAULT_FORM);
  ...
  const completed = useMemo(
    () =>
      [form.checklist_date, form.reviewed_by].every(
        (value) => typeof value === "string" && value.trim().length > 0
      ) &&
      form.modules_reviewed &&
      form.policy_updates_reviewed &&
      form.competency_validated,
    [form]
  );

  async function save(nextStatus: "draft" | "finalized") {
    if (nextStatus === "finalized" && !completed) {
      setError("Complete required checklist items before finalizing.");
      return;
    }
    ...
    const payload = {
      ...
      form_type: "annual_training_checklist",
      form_title: "Annual Training Checklist",
      status: nextStatus,
    ...
    <div className="space-y-6 p-6">
      <div className="overflow-hidden rounded-[28px] border border-sky-100 bg-gradient-to-br from-sky-50 via-white to-cyan-50 shadow-[0_20px_60px_-25px_rgba(2,132,199,0.35)]">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">
                Complete yearly training checklist items, save progress, then finalize.
              </p>
            </div>
        ...
        <div className="space-y-6 px-6 py-6 md:px-8">
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-700">Checklist Date</label>
            <input
              type="date"
              value={form.checklist_date}
              onChange={(e) => setForm((prev) => ({ ...prev, checklist_date: e.target.value }))}
          ...
          <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
            <label className="flex items-center gap-3 text-sm font-semibold text-slate-700">
              <input
                type="checkbox"
                checked={form.modules_reviewed}
                onChange={(e) => setForm((prev) => ({ ...prev, modules_reviewed: e.target.checked }))}
                disabled={status === "finalized"}
                className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
              />
              Required modules reviewed
            </label>
            <label className="flex items-center gap-3 text-sm font-semibold text-slate-700">
              <input
                type="checkbox"
                checked={form.policy_updates_reviewed}
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, policy_updates_reviewed: e.target.checked }))
                }
                disabled={status === "finalized"}
                className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
              />
            ...
            <label className="flex items-center gap-3 text-sm font-semibold text-slate-700">
              <input
                type="checkbox"
                checked={form.competency_validated}
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, competency_validated: e.target.checked }))
                }
                disabled={status === "finalized"}
                className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
              />
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-700">Reviewed By</label>
            <input
              type="text"
          ...
          {event ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
              <p>
                ...
                {event.event_title || "Annual Training Checklist"}
              </p>
              <p className="mt-1">
        ...
        <div className="mt-4 space-y-3">
          {history.length === 0 ? (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">
              No annual training checklist history yet.
            </div>
          ) : (
      ...
      {recordId ? <div className="text-xs text-slate-500">Record ID: {recordId}</div> : null}
    </div>
  );
}

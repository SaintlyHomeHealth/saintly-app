...
export async function requireSalesAgentChatAccess(): Promise<StaffProfile> {
  const staff = await requireSalesAgentWorkspaceShell();
  const access = resolveEffectivePageAccess(staff);
  if (!access.workspace_sales_agent_chat) {
    redirect("/unauthorized?reason=forbidden");
  }
  return staff;
}

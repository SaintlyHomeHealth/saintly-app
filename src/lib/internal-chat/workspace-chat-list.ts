...
export type WorkspaceInternalChatListItem = {
  id: string;
  chatType: string;
  title: string;
...
/**
 * Shared list builder for /api/workspace/internal-chat/chats and the chat RSC page.
 * Caller must enforce auth (workspace internal chat access).
 */
export async function getWorkspaceInternalChatListForStaff(
  staff: StaffProfile
): Promise<WorkspaceInternalChatListItem[]> {
  ...
  const lastMsgByChat = new Map<string, NonNullable<typeof msgRows>[number]>();
  ...
  const out = chatIds
    .map((id) => {
      ...
      } as WorkspaceInternalChatListItem;
    })
    .filter(Boolean) as WorkspaceInternalChatListItem[];

...
const LOG = "[recruiting-leads-notify]";
...
/**
 * Staff who should receive Facebook PT recruiting lead alerts:
 * - Has Recruiting page access
 * - Manager-tier or above (includes recruiter preset roles)
 * - Never sales_agent
 */
export async function resolveRecruitingLeadsNotifyUserIds(supabase: SupabaseClient): Promise<string[]> {
  const { data: rows, error } = await supabase
    .from("staff_profiles")
    .select("user_id, role, page_access_preset, page_permissions, admin_shell_access, push_notifications_enabled, is_active")
    .eq("is_active", true);
  ...
  for (const raw of rows ?? []) {
    ...
    if (access.recruiting !== true) continue;
